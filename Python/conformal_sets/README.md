# Notes on Conformal Sets

Note the `NIJRecid.csv` file is created with the below code.
    ...
    pdata.to_csv("NIJRecid.csv",index=False)

This is the same data from the NIJ recidivism contest, and I use this simple feature engineering to illustrate various things, see for example [Variance of leaderboard metrics for competitions](https://andrewpwheeler.com/2021/07/24/variance-of-leaderboard-metrics-for-competitions/). 

This isn't exactly the same as the model we used in the NIJ competition, but is close.